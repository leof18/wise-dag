...
var Graph = Class.extend({
  ...
  init: function (s) {
    ...
    _.each(
      this.managed_vertex_property_names,
      function (p) {
  ...
  deleteVertex: function (v) {
    ...
    this.edges = _.filter(this.edges, function (e) {
      return !(
        _.contains(v.incomingEdges, e) || _.contains(v.outgoingEdges, e)
      );
    });
  ...
  contractVertex: function (v0) {
    ...
    _.each(children, function (v) {
      _.each(children, function (w) {
        if (v.id != w.id) self.addEdge(w, v, Graph.Edgetype.Bidirected);
      }); // v <- v0 -> w
      _.each(parents, function (w) {
        if (v.id != w.id) self.addEdge(w, v, Graph.Edgetype.Directed);
      }); // v <- v0 <- w
      _.each(spouses, function (w) {
        if (v.id != w.id) self.addEdge(w, v, Graph.Edgetype.Bidirected);
      }); // v <- v0 <-> w
      _.each(neighbours, function (w) {
        if (v.id != w.id) self.addEdge(w, v, Graph.Edgetype.Directed);
      }); // v <- v0 - w
    });
    _.each(parents, function (v) {
      _.each(neighbours, function (w) {
        if (v.id != w.id) self.addEdge(v, w, Graph.Edgetype.Directed);
      }); // v -> v0 - w
    });
    _.each(spouses, function (v) {
      _.each(neighbours, function (w) {
        if (v.id != w.id) self.addEdge(v, w, Graph.Edgetype.Bidirected);
      }); // v <-> v0 - w
    });
    _.each(neighbours, function (v) {
      _.each(neighbours, function (w) {
        if (v.id != w.id) self.addEdge(v, w, Graph.Edgetype.Undirected);
      }); // v - v0 - w
    });
  ...
  districtOf: function (vertex_array, clear_visited_function) {
    return this.transitiveClosureOf(
      vertex_array,
      "getSpouses",
      clear_visited_function
    );
  ...
  anteriorsOf: function (vertex_array, clear_visited_function) {
    return this.transitiveClosureOf(
      vertex_array,
      "getPossibleParentsAndPossibleNeighbours",
      clear_visited_function
    );
  },
  ...
  areAdjacent: function (v1, v2) {
    ...
    if (
      _.any(
        _.map(v1.outgoingEdges, function (e) {
          return e.v2 == v2;
        })
      )
    ) {
    ...
    return _.any(
      _.map(v1.incomingEdges, function (e) {
        return e.v1 == v2;
      })
    );
  },
  ...
  visitAllPathsBetweenVisitedNodesInTree: function () {
    ...
    while (q.length > 0) {
      v = q.pop();
      var children = _.reject(v.getNeighbours(), function (v2) {
        return v2 === root || v2.traversal_info.depth > 0;
      });
      _.each(children, function (v2) {
        v2.traversal_info.depth = v.traversal_info.depth + 1;
        if (Graph.Vertex.isVisited(v2) && v2.traversal_info.depth > max_depth) {
    ...
    var nr_tokens = 0;
    _.chain(this.vertices.values())
      .filter(Graph.Vertex.isVisited)
      .each(function (v) {
    ...
    while (nr_tokens > 1) {
      ...
      if (
        v.traversal_info.parent &&
        !Graph.Vertex.isVisited(v.traversal_info.parent)
      ) {
  ...
  toAdjacencyList: function () {
    ...
    _.each(g.vertices.values(), function (v) {
      var children = v.getChildren(),
        ...
        spouses = v.getSpouses();
      var r = "",
        rc = [];
      if (children.length + neighbours.length + spouses.length > 0) {
  ...
  toVertexLabels: function () {
    var expandLabel = function (v, g) {
      ...
      return (
        ...
        " " +
        property_string +
        (v.layout_pos_x !== undefined
          ? " @" + v.layout_pos_x.toFixed(3) + "," + v.layout_pos_y.toFixed(3)
          : "")
      );
    };
    var r = "";
  ...
  sourceConnectedToTarget: function () {
    ...
    if (arguments.length == 0) {
      ...
      _.each(
        ...
        function (v) {
          this.getVertex(v) &&
            (this.getVertex(v).traversal_info.visited = true);
        },
        this
      );
      return this.sourceConnectedToTarget(this.getSource(), this.getTarget());
      ...
      if (
        s.getChildren().any(function (n) {
          return (
            !n.traversal_info.visited &&
            !n.traversal_info.adjusted_for &&
            this.sourceConnectedToTarget(n, t)
          );
        }, this)
      ) {
        return true;
...
(function (c) {
  _.each(c.prototype.managed_vertex_property_names, function (p) {
    var pcamel = p.substring(0, 1).toUpperCase() + p.substring(1, p.length);
    c.prototype["is" + pcamel] = function (v) {
      return this.vertexHasProperty(v, p);
    };
    c.prototype["add" + pcamel] = function (v) {
      return this.addVertexProperty(v, p);
    };
    c.prototype["remove" + pcamel] = function (v) {
      return this.removeVertexProperty(v, p);
    };
    c.prototype["get" + pcamel + "s"] = function () {
      return this.getVerticesWithProperty(p);
    };
    c.prototype["set" + pcamel + "s"] = function (vs) {
      ...
      _.each(
        vs,
        function (v) {
          this.addVertexProperty(v, p);
        },
        this
      );
    };
    c.prototype["removeAll" + pcamel + "s"] = function () {
      return this.removePropertyFromAllVertices(p);
    };
  });
})(Graph);
...
Graph.Vertex = Class.extend({
  ...
  getPossibleNeighbours: function () {
    return this.getKinship(Graph.Edgetype.Undirected).concat(
      this.getKinship(Graph.Edgetype.Unspecified)
    );
  },
  ...
  getPossibleParents: function () {
    return this.getKinship(Graph.Edgetype.Directed, false).concat(
      this.getKinship(Graph.Edgetype.PartDirected, false)
    );
  },
...
Graph.Edge = Class.extend({
  ...
  toString: function () {
    ...
    if (Graph.Edgetype.Symmetric[this.directed] && v1id > v2id) {
      var tmp = v1id;
      v1id = v2id;
      v2id = tmp;
    }
...
var GraphAnalyzer = {
  ...
  equals: function (g, h) {
    ...
    var acanon = function (a) {
      var tmp;
      if (Graph.Edgetype.Symmetric[a[2]] && a[0] < a[1]) {
        tmp = a[1];
        a[1] = a[0];
        a[0] = tmp;
      }
  ...
  trekRule: function (g, v1, v2, use_ids_as_labels, standardized) {
    ...
    visit(gtrek.getVertex("up_" + v1.id), gtrek.getVertex("dw_" + v2.id), [
      "up_" + v1.id,
    ]);
    ...
    for (i = 0; i < treks.length; i++) {
      ...
      for (j = 0; j < treks[i].length - 1; j++) {
        ...
        if (v1_pre == v2_pre) {
          ...
          if (v1_id == v2_id) {
            if (!standardized) {
              trek_monomials[i].push(/*<->*/ "v" + vnr[v1_id]);
            }
          } else {
            trek_monomials[i].push(
              /*<-->*/
  ...
  vanishingTetrads: function (g, nr_limit, type) {
    ...
    for (var i = 0; i < latents.length; i++) {
      is_latent[latents[i].id] = 1;
    }

    var vv = _.pluck(
        _.reject(g.getVertices(), function (v) {
          return is_latent[v.id];
        }),
        "id"
      ),
      i1,
    ...
    function examineQuadruple(i1, i2, j1, j2) {
      var pi, pj;
      iside = [gtrek.getVertex("up_" + i1), gtrek.getVertex("up_" + i2)];
      jside = [gtrek.getVertex("dw_" + j1), gtrek.getVertex("dw_" + j2)];
      if (type == "between" || type == "epistemic") {
        pi = _.pluck(g.parentsOf(g.getVertex([i1, i2])), "id");
        pj = _.pluck(g.parentsOf(g.getVertex([j1, j2])), "id");
        if (
          type == "between" &&
          (pi.length != 1 || pj.length != 1 || pi[0] == pj[0])
        ) {
          return;
        }
        if (
      ...
      for (i2 = i1 + 1; i2 < vv.length; i2++) {
        ...
        for (i3 = i2 + 1; i3 < vv.length; i3++) {
          ...
          for (i4 = i3 + 1; i4 < vv.length; i4++) {
            ...
            if (type == "within" && _.intersection(p1, p2, p3, p4).length == 0)
              continue;
  ...
  minVertexCut: function (g, sources, targets) {
    ...
    for (i = 0; i < sources.length; i++) {
      ...
      for (j = 0; j < targets.length; j++) {
        t = targets[j];
        if (s.id == t.id || g.getEdge(s.id, t.id)) return undefined;
        is_target[t.id] = true;
      }
    }
  ...
  isAdjustmentSetDirectEffect: function (g, Z) {
    ...
    if (gtype != "dag") {
      throw (
        "Cannot compute adjustment sets for direct effects for graph of type " +
        gtype
      );
    }
    ...
    return !this.dConnected(
      gbd,
      gbd.getSources(),
      gbd.getTargets(),
      _.map(Z, Graph.getVertex, gbd)
    );
  },
  ...
  listMsasTotalEffect: function (g, M, F, max_nr) {
    ...
    if (gtype != "dag" && gtype != "pdag" && gtype != "mag" && gtype != "pag") {
      throw (
        "Cannot compute total affect adjustment sets for graph of type " + gtype
      );
    }

    if (!g || g.getSources().length < 1 || g.getTargets().length < 1) {
      return [];
    }
    ...
    if (GraphAnalyzer.violatesAdjustmentCriterion(gg)) {
      return [];
    }
    ...
    if (S.length > 0) {
      if (
        this.dConnected(gbd, gbd.getTargets(), _.map(S, Graph.getVertex, gbd))
      ) {
        return [];
      } else {
        _.each(S, function (s) {
          gbd.removeSelectedNode(s);
          gbd.addAdjustedNode(s);
        });
      }
    }
    ...
    var latent_nodes = _.map(
      gg.getLatentNodes().concat(this.dpcp(gg)),
      Graph.getVertex,
      gam
    );
    ...
    // values.
    var r = this.listMinimalSeparators(
      gam,
      adjusted_nodes,
      _.compact(latent_nodes),
      max_nr
    );
  ...
  canonicalAdjustmentSet: function (g) {
    var Z = _.difference(
      g.anteriorsOf(_.union(g.getSources(), g.getTargets())),
      _.union(
        g.getLatentNodes(),
        g.getSources(),
        g.getTargets(),
  ...
  listMsasDirectEffect: function (g, must, must_not) {
    ...
    if (gtype != "dag") {
      throw (
        "Cannot comute direct-effect adjustment sets for graphs of type " +
        gtype
      );
    }
    if (gtype == "pdag") {
    ...
    var de_y = g.descendantsOf(
      _.intersection(g.descendantsOf(g.getSources()), g.getTargets())
    );
    ...
    var gam = GraphTransformer.moralGraph(
      GraphTransformer.ancestorGraph(GraphTransformer.indirectGraph(g))
    );

    if (must) adjusted_nodes = adjusted_nodes.concat(must);
    if (must_not) latent_nodes = latent_nodes.concat(must_not);
  ...
  listDseparators: function (g, must, must_not, max_nr) {
    ...
    return this.listMinimalSeparators(
      gam,
      adjusted_nodes,
      latent_nodes,
      max_nr
  ...
  listBasisImplications: function (g) {
    ...
    _.each(vv, function (v) {
      ...
      if (sepnodes.length > 0) {
        r.push([v.id, _.pluck(sepnodes, "id"), [parents]]);
      }
    });
    return r;
  },
  ...
  listMinimalImplications: function (g, max_nr) {
    ...
    for (i = 0; i < vv.length; i++) {
      for (var j = i + 1; j < vv.length; j++) {
        if (
          !g2.isLatentNode(vv[i]) &&
          !g2.isLatentNode(vv[j]) &&
          !g2.getEdge(vv[i].id, vv[j].id) &&
          !g2.getEdge(vv[j].id, vv[i].id)
        ) {
  ...
  isDG: function (g) {
    var cpdag = GraphTransformer.dependencyGraph2CPDAG(g),
      p1,
      p2;
    if (
      g.edges.all(function (e) {
        if (
          typeof cpdag.getEdge(e.v1.id, e.v2.id, Graph.Edgetype.Undirected) ==
          "undefined"
        ) {
  ...
  directEffectEqualsTotalEffect: function (g) {
    return (
      _.chain(g.childrenOf(g.getSources()))
        .difference(g.getSources())
        .difference(g.getTargets())
        .value().length == 0
    );
  },
  ...
  dpcp: function (g, X, Y) {
    ...
    return g.getVertex(
      _.pluck(
        gn.posteriorsOf(
          _.difference(this.properPossibleCausalPaths(gn, X, Y), X)
        ),
        "id"
      )
    );
  ...
  nodesThatViolateAdjustmentCriterionWithoutIntermediates: function (g) {
    ...
    var set_causal = function (v) {
      is_on_causal_path[v.id] = true;
    };
    _.each(this.properPossibleCausalPaths(g), set_causal);
    var is_violator = function (v) {
      return g.isAdjustedNode(v) && !is_on_causal_path[v.id];
    };
  ...
  visitGraph: function (g, startNodes, onCanVisitEdge, onIsFinalNode) {
    ...
    if (!onCanVisitEdge)
      onCanVisitEdge = function () {
        return true;
      };
    if (!onIsFinalNode)
      onIsFinalNode = function () {
        return false;
      };

    var q_from_parent = [],
      q_from_child = startNodes;
    var visited_from_parent = {},
      visited_from_child = {};
    var v;
    var from_parents;
    ...
    function visitEdge(e) {
      ...
      var arrowHeadAtT =
        (e.directed == Graph.Edgetype.Directed && outgoing) ||
  ...
  closeSeparator: function (g, y, z, anteriors, blockable_nodes) {
    ...
    if (blockable_nodes) {
      ...
      _.each(blockable_nodes, function (v) {
        blockable_nodes_hash[v.id] = true;
      });
      _.each(y, function (v) {
        blockable_nodes_hash[v.id] = false;
      });
      isBlockableNode = function (v) {
        return blockable_nodes_hash[v.id];
      };
    } else {
      var start = {};
      _.each(y, function (v) {
      ...
      isBlockableNode = function (v) {
        return !start[v.id] && !g.isLatentNode(v);
      };
    }
  ...
  ancestralInstrument: function (g, x, y, z, g_bd, de_y) {
    if (arguments.length < 5) {
      g_bd = GraphTransformer.backDoorGraph(g, [x], [y]);
    }
    y = g_bd.getVertex(y.id);
    z = g_bd.getVertex(z.id);
    ...
    if (_.intersection(W, de_y).length > 0) {
      return false;
    }
    if (_.intersection(W, [x]).length === 1) {
      return false;
    }
  ...
  conditionalInstruments: function (g, x, y) {
    ...
    var i,
      r = [],
      W;
    var g_bd = GraphTransformer.backDoorGraph(g, [x], [y]);
  ...
  listMinimalSeparators: function (g, _must, _must_not, max_nr) {
    ...
    _.each(g.neighboursOf(g.getTargets()), function (v) {
      Uh.set(v.id, v);
    });
    var U = Uh.values();
    ...
    var must = [];
    if (_must) {
      _.each(_must, function (v) {
        must.push(g.getVertex(v.id));
      });
    }
    var must_not = [];
    if (_must_not) {
      _.each(_must_not, function (v) {
        must_not.push(g.getVertex(v.id));
      });
    }
    ...
    var listMinSep = function (A, U) {
      ...
      var Astar = GraphAnalyzer.connectedComponentAvoiding(
        g,
        g.getSources(),
        SA.concat(must)
      );
  ...
  bottleneckNumbers: function (g, topological_index) {
    ...
    var bottleneck_number = {},
      reaches_source = {},
      reaches_target = {},
      visited = {};
    _.each(
      g.ancestorsOf(g.getSources(), function () {
    ...
    _.each(
      g.ancestorsOf(g.getTargets(), function () {
        var adj = g.getAdjustedNodes();
        g.clearTraversalInfo();
        _.each(adj, function (v) {
          Graph.Vertex.markAsVisited(v);
    ...
    _.each(vv, function (v) {
      if (reaches_source[v.id] && !reaches_target[v.id]) {
        bottleneck_number[v.id] = bn_s;
      } else if (!reaches_source[v.id] && reaches_target[v.id]) {
    ...
    var visit = function (v) {
      if (!visited[v.id] && (reaches_source[v.id] || reaches_target[v.id])) {
        visited[v.id] = true;
        var children = _.filter(v.getChildren(), function (v) {
          return reaches_source[v.id] || reaches_target[v.id];
        });
        _.each(children, visit);
        if (
          children.length > 1 &&
          _.some(children, function (v2) {
            return (
              bottleneck_number[v2.id] != bottleneck_number[children[0].id]
            );
          })
        ) {
  ...
  blockTree: function (g, bicomps) {
    ...
    _.each(bicomps, function (edge_list) {
      bt.addVertex(
        new Graph.Vertex({ id: "C" + edge_list[0].component_index })
      );
    });
    _.each(g.vertices.values(), function (v) {
    ...
    var visit = function (v) {
      ...
      _.each(v.getNeighbours(), function (n) {
        ...
        if (w.traversal_info.dtime == 0) {
          ...
          if (
            w.traversal_info.dtime < v.traversal_info.dtime &&
            w != v.traversal_info.parent
          ) {
            // (v,w) is a back edge
            q.push(e);
  ...
  lexicographicBreadthFirstSearch: function (g, componentV) {
    ...
    _.each(componentV, function (v) {
      componentSet.set(v.id, v);
    });
    ...
    var firstSet = {
      ...
      prevIteration: 0,
      set: componentSet,
      setKeys: componentV.map(function (v) {
        return v.id;
      }), //cache of this.set.keys(), not updated for removed elements
    ...
    _.each(componentV, function (v) {
      containedSet.set(v.id, firstSet);
    });
    ...
    while (firstSet) {
      while (
        firstSet.setKeyIndex < firstSet.setKeys.length &&
  ...
  isChordal: function (g, componentV) {
    ...
    _.each(ordering, function (v, i) {
      positions.set(v.id, i);
    });
    return _.every(ordering, function (v, i) {
      var j = _.max(
        _.map(v.getNeighbours(), function (w) {
  ...
  validate: function (g) {
    switch (g.getType()) {
      case "dag":
        if (
          !_.every(g.getEdges(), function (e) {
            return (
              e.directed == Graph.Edgetype.Directed ||
              e.directed == Graph.Edgetype.Bidirected
            );
          })
        ) {
      ...
      case "graph":
        return _.every(g.getEdges(), function (e) {
          return e.directed == Graph.Edgetype.Undirected;
        });
      case "digraph":
        return true;
      case "pdag":
    ...
    for (var i = 0; i < vv.length; i++) {
      ...
      if (c !== undefined) {
        ...
        for (j = 0; j < c.length; j++) {
          if (v_count[c[j]] > 1) {
            return c
              .slice(c.indexOf(c[j]), c.lastIndexOf(c[j]) + 1)
              .join("&rarr;");
          }
        }
      }
  ...
  containsSemiCycle: function (g) {
    return GraphAnalyzer.containsCycle(
      GraphTransformer.contractComponents(
        g,
        GraphAnalyzer.connectedComponents(g),
  ...
  isEdgeStronglyProtected: function (g, e) {
    ...
    for (i = 0; i < bdp.length; i++) {
      if (bdp[i].id != a.id) {
        if (
          !g.areAdjacent(bdp[i], a) ||
          g.getEdge(a, bdp[i], Graph.Edgetype.Directed)
        ) {
          return true;
        }
      }
  ...
  isEdgeVisible: function (g, e) {
    ...
    if (
      _.difference(
        _.union(e.v1.getParents(), e.v1.getSpouses()),
        e.v2.getAdjacentNodes()
      ).length > 0
    ) {
      return true;
    }
    ...
    var vpar = GraphTransformer.inducedSubgraph(
      g,
      _.union([e.v2], e.v2.getParents())
    );
    vpar = g.getVertex(
      _.pluck(vpar.districtOf([vpar.getVertex(e.v1.id)]), "id")
    );

    if (
      _.difference(
        _.union(g.parentsOf(vpar), g.spousesOf(vpar)),
        e.v2.getAdjacentNodes()
      ).length > 0
    ) {
      return true;
  ...
  treeID: function (g) {
    ...
    function nodeidx(v) {
      return topology[v.id] - 1;
    }

    var oldpa = _.map(toponodes, function (v) {
      return v ? v.getParents() : null;
    });
    ...
    _.each(oldtoponodes, function (v, i) {
      ...
      if (
        p &&
        (p.length == 1 || (p.length == 0 && v.getChildren().length > 0))
      ) {
        pa.push(p);
        toponodes.push(v);
      }
    ...
    var B = this.graphToAdjacencyMatrix(
      g,
      toponodes,
    ...
    var omega = _.map(toponodes, function (xxx, f) {
      return _.map(toponodes, function (xxx, t) {
        return f == t || B[f][t]
          ? MPoly("w" + Math.min(f, t) + "_" + Math.max(f, t))
          : null;
      });
    });
    ...
    var sigmaexpand = _.map(new Array(n), function (xxx, i) {
      return new Array(n);
    });
    var sigmaevalobj = {};
    for (i = 0; i < n; i++)
      for (j = i; j < n; j++) {
        ...
        //console.log(treks)
        var terms = _.map(treks, function (t) {
          return _.map(t, function (e) {
            if (e instanceof Graph.Vertex) return omega[nodeidx(e)][nodeidx(e)];
            else if (e.directed == Graph.Edgetype.Bidirected)
              return omega[nodeidx(e.v1)][nodeidx(e.v2)];
            else return lambda[nodeidx(e.v1)][nodeidx(e.v2)];
          });
        });
        var trekProducts = _.map(terms, function (t) {
          return _.reduce(t, function (a, b) {
            return a.mul(b);
          });
        });
        var treksum = _.reduce(trekProducts, function (a, b) {
          return a.add(b);
        });
    ...
    function simulateNumeric(variableValue) {
      ...
      for (i = 0; i < n; i++)
        for (j = i; j < n; j++) {
          evaled[sigma[i][j].toString()] =
            sigmaexpand[i][j].evalToBigInt(inputs);
        }
      //console.log(evaled)
      return evaled;
    }
    ...
    var simulated = [
      simulateNumeric(function () {
        return Math.floor((1 << 52) * Math.random()) + 1;
      }),
      simulateNumeric(function (i) {
        return primes[3 * i + 20];
      }),
    ...
    var FASTP = {
      ...
      toString: function () {
        function polyToString(p, sigmaWithIndices) {
          ...
          var p = p.toString();
          if (!sigmaWithIndices)
            p = p.replace(/(s)([0-9]+)_([0-9]+)/g, function (x, y, i, j) {
    ...
    function isZeroSigmaPoly(p) {
      ...
      for (var i = 0; i < simulated.length; i++)
        if (p.evalToBigInt(simulated[i]) != 0) return false;
    ...
    function propagate(i) {
      ...
      _.each(missingSpouses[i], function (j) {
        ...
        if (
          _.find(ID[i].fastp, function (fastp) {
        ...
        var newfastp = _.map(ID[i].fastp, function (fastp) {
          ...
          return FASTP.make(
            fastp.r().mul(si2).add(fastp.p().mul(si1)),
            fastp.s() ? fastp.t().mul(si2).add(fastp.q().mul(si1)) : null,
            fastp.r().mul(si4).add(fastp.p().mul(si3)),
            fastp.s() ? fastp.t().mul(si4).add(fastp.q().mul(si3)) : null,
            fastp.s()
          );
        });
        ID[j] = {
          propagate: i,
          propagatePath:
            "propagate" in ID[i] ? ID[i].propagatePath.concat([i]) : [i],
          fastp: newfastp,
          propagatedMissingCycles:
            "propagate" in ID[i]
              ? ID[i].propagatedMissingCycles
              : ID[i].missingCycles,
      ...
      if (_.isNull(pa[r]))
        for (i = r + 1; i < n; i++)
          if (!B[r][i] && !_.isNull(pa[i]) && !sigmaexpand[r][pa[i]].isZero()) {
            ID[i] = {
              instrument: r,
              fastp: [FASTP.makeFraction(sigma[r][i], sigma[r][pa[i]])],
            };
            propagate(i);
          }
    ...
    function missingCycleToQuadraticEquation(cycle) {
      ...
      var eqs = _.map(new Array(k), function (x, i) {
        return bidiEquation(cycle[i], cycle[i + 1]);
      });
      while (k > 2) {
      ...
      if (k == 2) {
        eqs[0] = [
          DET2(eqs[0][A], eqs[1][C], eqs[1][A], eqs[0][B]),
    ...
    function solveQuadraticEquation(abc) {
      ...
      return [
        FASTP.make(minus_b, MINUS_ONE, two_a, ZERO, ss),
    ...
    function fastpMightSatisfyQuadraticEquation(fastp, abc) {
      ...
      for (var i = 0; i < simulated.length; i++) {
        ...
        var t = fastp.t();
        if (!s)
          return isZeroSigmaPoly(ADD(MUL(a, p, p), MUL(b, p, r), MUL(c, r, r)));
        ...
        var app_aqqs_bpr_bqst_crr_ctts =
          a * p * p +
          a * q * q * s +
          b * p * r +
          b * q * s * t +
        ...
        if (app_aqqs_bpr_bqst_crr_ctts < 0 && minus_2apq_bpt_bqr_2crt > 0)
          return false;
        if (app_aqqs_bpr_bqst_crr_ctts > 0 && minus_2apq_bpt_bqr_2crt < 0)
          return false;
        if (app_aqqs_bpr_bqst_crr_ctts != 0 && minus_2apq_bpt_bqr_2crt == 0)
          return false;
        if (
          app_aqqs_bpr_bqst_crr_ctts == 0 &&
          minus_2apq_bpt_bqr_2crt != 0 &&
          s != 0
        )
          return false;
        if (
          app_aqqs_bpr_bqst_crr_ctts ** 2n !=
          minus_2apq_bpt_bqr_2crt ** 2n * s
        )
          return false;
      }
    ...
    function solveMissingCycle(cycle) {
      ...
      if (aIsZero) {
        ID[i] = {
          missingCycles: [cycle.slice()],
          fastp: [FASTP.makeFraction(abc[2].negate(), abc[1])],
        };
      } else if (!(i in ID)) {
        ID[i] = {
          missingCycles: [cycle.slice()],
          fastp: solveQuadraticEquation(abc),
        };
        // alert(fastpMightSatisfyQuadraticEquation(ID[i].fastp[0], abc))
      } else {
        var newfastp = _.filter(ID[i].fastp, function (fastp) {
          return fastpMightSatisfyQuadraticEquation(fastp, abc);
        });
      ...
      return _.find(missingSpouses[e], function (j) {
        ...
        if (visitedInCycle[j]) {
          if (j == s && cyclePrefix.length == forceCycleLength) {
            cyclePrefix.push(j);
            if (solveMissingCycle(cyclePrefix)) return true;
            cyclePrefix.pop();
          }
          return false;
        }
    ...
    for (forceCycleLength = 3; forceCycleLength < n; forceCycleLength++) {
      ...
      for (i = 0; i < n; i++) {
        ...
        allEdgesIdentified =
          allEdgesIdentified && newPossibleSolutionCount == 1;
      }
    ...
    for (i = 0; i < n; i++)
      if (i in ID) {
        ...
        if ("instrument" in ID[i])
          identification.instrument = toponodes[ID[i].instrument].id;
        if ("propagate" in ID[i])
          identification.propagate = toponodes[ID[i].propagate].id;
        if ("propagatePath" in ID[i])
          identification.propagatePath = nodeIdxToNodeIdArray(
            ID[i].propagatePath
          );
        _.map(
          ["missingCycles", "propagatedMissingCycles", "oldMissingCycles"],
          function (mcid) {
            if (mcid in ID[i])
              identification[mcid] = _.map(ID[i][mcid], nodeIdxToNodeIdArray);
          }
        );
    ...
    if (hasnontreenodes)
      res.warnings = [
        "Some nodes have more than one parent. TreeID assumes all nodes except one (the root) have exactly one parent, and ignores nodes with more parents.",
      ];
    return res;
  },
};
...
GraphLayouter.prototype = {
  layoutCalcBounds: function () {
    ...
    var xpad = Math.max((maxx - minx) * 0.1, 0.5);
    maxx += xpad;
    minx -= xpad;
    var ypad = Math.max((maxy - miny) * 0.1, 0.5);
    maxy += ypad;
    miny -= ypad;
    var trimn = function (n) {
...
var GraphParser = {
  ...
  parseDot: function (code, g) {
    ...
    var isdot = code
      .trim()
      .match(/^(digraph|graph|dag|pdag|mag|pag)(\s+\w+)?\s*\{([\s\S]*)\}$/im);
  ...
  parseDotStatementArray: function (statements, g) {
    ...
    var v = function (id) {
      if (id == "graph" || id == "node") {
        throw (
          "Syntax error: variables cannot be named 'graph' or 'node'. " +
    ...
    var parse_position = function (s) {
      ...
      if (typeof tok[1] !== "string" || typeof tok[2] !== "string") {
    ...
    return _.map(vsub.getVertices(), function (v) {
  ...
  parseVertexLabelsAndWeights: function (vertexLabelsAndWeights) {
    ...
    for (i = 0; i < lines.length; i++) {
      ...
      g.addVertex(new Graph.Vertex({ id: vid, weight: weight }));
  ...
  parseAdjacencyMatrix: function (adjacencyMatrix, vertexLabelsAndWeights) {
    ...
    if (parseInt(n, 10) !== n || n !== g.getNumberOfVertices()) {
      throw (
        "Error loading data: Adjacency matrix is not square or does not match number of vertices: " +
        n +
        " , " +
...
var GraphTransformer = {
  ...
  inducedSubgraph: function (g, vertex_array) {
    ...
    for (i = 0; i < g.edges.length; i++) {
      var e = g.edges[i];
      if (
        _.contains(other_vertex_ids, e.v1.id) &&
  ...
  measurementPart: function (g) {
    ...
    for (i = 0; i < g.edges.length; i++) {
      var e = g.edges[i];
      if (e.directed == Graph.Edgetype.Directed && !g.isLatentNode(e.v2)) {
        gn.addEdge(e.v1.id, e.v2.id, e.directed);
      }
      if (
        e.directed == Graph.Edgetype.Bidirected &&
  ...
  edgeInducedSubgraph: function (g, edge_array) {
    ...
    for (var i = 0; i < edge_array.length; i++) {
      var edge_other = edge_array[i];
      var edge_my = g.getEdge(
        edge_other.v1.id,
        edge_other.v2.id,
        edge_other.directed
      );
      if (edge_my) {
        if (!gn.getVertex(edge_my.v1.id)) gn.addVertex(edge_my.v1.id);
        if (!gn.getVertex(edge_my.v2.id)) gn.addVertex(edge_my.v2.id);
  ...
  indirectGraph: function (g, X, Y) {
    ...
    if (arguments.length == 3) {
      _.each(X, function (v) {
        gback.addSource(v.id);
      });
      _.each(Y, function (v) {
        gback.addTarget(v.id);
      });
    }
    ...
    _.each(ee, function (e) {
      gback.deleteEdge(e.v1, e.v2, e.directed);
    });
    return gback;
  },
  ...
  causalFlowGraph: function (g, X, Y) {
    ...
    var clearVisitedWhereNotAdjusted = function () {
      _.each(g.vertices.values(), function (v) {
        if (g.isAdjustedNode(v)) Graph.Vertex.markAsVisited(v);
        else Graph.Vertex.markAsNotVisited(v);
      });
    };
    return this.inducedSubgraph(
      g,
      _.intersection(
        g.ancestorsOf(Y, clearVisitedWhereNotAdjusted),
  ...
  simpleOpenPaths: function (g, Z) {
    ...
    var target_ancestors_except_ancestors_of_violators = g.ancestorsOf(
      g.getTargets(),
      function () {
        ...
        g.clearTraversalInfo();
        _.each(an_violators, function (v) {
          Graph.Vertex.markAsVisited(v);
        });
      }
    );
    var intermediates_after_source = _.intersection(
      g.childrenOf(g.getSources()),
    ...
    var bottleneck_number = GraphAnalyzer.bottleneckNumbers(
      g,
      topological_index
    );
    ...
    for (var comp_i = 0; comp_i < comps.length; comp_i++) {
      ...
      if (comp.length > 1) {
        ...
        var current_component = GraphTransformer.inducedSubgraph(g_chain, comp);
        ...
        var current_block_tree = GraphAnalyzer.blockTree(
          current_component,
          bicomps
        );
        ...
        var visited_components = _.filter(
          current_block_tree.vertices.values(),
          function (v) {
    ...
    _.each(nodes_to_be_retained, function (v) {
      retain[v.id] = true;
    });
  ...
  activeBiasGraphWithoutSelectionNodes: function (g, opts) {
    ...
    opts = _.defaults(opts || {}, { direct: false });
    if (opts.X) {
      _.each(opts.X, function (v) {
        g.addSource(g.getVertex(v));
      });
    }
    if (opts.Y) {
      _.each(opts.Y, function (v) {
        g.addTarget(g.getVertex(v));
      });
    }
    ...
    if (opts.direct) {
      ...
      _.each(g.getSources(), function (s) {
        _.each(s.outgoingEdges, function (e) {
          if (
            g.isSource(e.v2) ||
            _.contains(intermediates_after_source, e.v2)
          ) {
            g_chain.deleteEdge(e.v1, e.v2, Graph.Edgetype.Directed);
          }
        });
    ...
    var w_nodes = _.reject(
      GraphAnalyzer.dpcp(g), // See Shpitser et al, UAI 2010
      function (w) {
        return !retain[w.id];
      }
    );
    var paths_to_violators = g.ancestorsOf(
      GraphAnalyzer.nodesThatViolateAdjustmentCriterionWithoutIntermediates(g)
    );
    g.clearTraversalInfo();
    _.each(w_nodes, Graph.Vertex.markAsVisited);
    _.each(
      w_nodes,
      function (w) {
        ...
        _.chain(paths_to_violators)
          .intersection(
            g.descendantsOf([w], preserve_previous_visited_information)
          )
          .each(function (v) {
            retain[v.id] = true;
          });
    ...
    // Delete all vertices that have not been marked for retention
    if (opts.direct) {
      _.each(intermediates_after_source, function (v) {
        retain[v.id] = true;
      });
    }
    _.each(vv, function (v) {
    ...
    var Lids = _.pluck(g_canon.L, "id"),
      Sids = _.pluck(g_canon.S, "id");
    (L = []), (S = []);
    ...
    if (opts.direct) {
      ...
      _.each(gind.edges, function (e) {
        if (
          e.directed == Graph.Edgetype.Directed &&
          !(
            (gind.isSource(e.v1) || gind.isTarget(e.v1)) &&
            (gind.isSource(e.v2) || gind.isTarget(e.v2))
          )
        ) {
  ...
  trekGraph: function (g, up_prefix, down_prefix) {
    ...
    for (i = 0; i < vv.length; i++) {
      vup = up_prefix + vv[i].id;
      vdown = down_prefix + vv[i].id;
    ...
    for (i = 0; i < ee.length; i++) {
      if (ee[i].directed == 2) {
        // bidirected edge
        vup = up_prefix + ee[i].v1.id;
        vdown = down_prefix + ee[i].v2.id;
        n.addEdge(vup, vdown);
        vup = up_prefix + ee[i].v2.id;
        vdown = down_prefix + ee[i].v1.id;
        n.addEdge(vup, vdown);
      }
    }
    for (i = 0; i < vv.length; i++) {
  ...
  canonicalDag: function (g) {
    ...
    _.each(g.getEdges(), function (e) {
      switch (e.directed) {
        case Graph.Edgetype.Undirected:
          while (rg.getVertex("S" + i)) {
            i++;
          }
          v = new Graph.Vertex({ id: "S" + i });
          rg.addVertex(v);
          rg.addSelectedNode(v);
          rg.addEdge(e.v2, v);
          rg.addEdge(e.v1, v);
          S.push(v);
          break;
  ...
  dagToMag: function (g, latent) {
    ...
    _.each(V, function (v) {
      ...
      _.each(V, function (u) {
        ...
        var reachable = GraphAnalyzer.visitGraph(
          g,
          [v],
          function (e, outgoing, from_parents) {
            var w = outgoing ? e.v1 : e.v2;
            if (from_parents && !outgoing) {
              //collider
              if (!Av[w.id] && !Au[w.id]) return false;
            } else if (w.id != v.id && w.id != u.id && !latentSet[w.id])
              return false;
            return true;
          },
          function (w) {
    ...
    _.each(comp, function (c) {
      ...
      for (var i = 0; i < comp_p.length; i++) {
        for (var j = i + 1; j < comp_p.length; j++) {
          mg.addEdge(comp_p[i].id, comp_p[j].id, Graph.Edgetype.Undirected);
        }
      }
    });
  ...
  vertexCapacityGraph: function (g) {
    ...
    _.each(g.vertices.values(), function (v) {
      ...
      if (g.getSource() !== v && g.getTarget() !== v) {
        gn.addEdge(
          new Graph.Edge.Directed({
            v1: gn.getVertex("I" + v.id),
            v2: gn.getVertex("O" + v.id),
            capacity: 1,
            is_backedge: false,
          })
        );
        gn.addEdge(
          new Graph.Edge.Directed({
            v2: gn.getVertex("I" + v.id),
            v1: gn.getVertex("O" + v.id),
            capacity: 0,
            is_backedge: true,
          })
        );
    ...
    _.each(g.edges, function (e) {
      if (e.v1 !== g.getTarget() && e.v2 !== g.getSource()) {
        gn.addEdge(
          new Graph.Edge.Directed({
            v1: gn.getVertex("O" + e.v1.id),
            v2: gn.getVertex("I" + e.v2.id),
            capacity: 1,
            is_backedge: false,
          })
        );
  ...
  transitiveReduction: function (g) {
    ...
    _.each(g.edges, function (e) {
      ...
      if (en) {
        ...
        if (
          _.intersection(g.descendantsOf([e.v1]), g.ancestorsOf([e.v2]))
            .length > 2
        ) {
  ...
  dagToCpdag: function (g) {
    ...
    while (changed) {
      ...
      for (var i = 0; i < es.length; i++) {
        if (
          es[i].directed == Graph.Edgetype.Directed &&
  ...
  dependencyGraph2CPDAG: function (g) {
    ...
    var vv = g.vertices.values();
    _.each(vv, function (v) {
      gn.addVertex(v.cloneWithoutEdges());
    });
    _.each(g.edges, function (e) {
  ...
  pathPairGraph: function (g) {
    ...
    topo_sort.sort(function (a, b) {
      return topological_index[a] < topological_index[b] ? -1 : 1;
    });
  ...
  cgToRcg: function (g) {
    ...
    function areConnected(v, w) {
      return _.some(v.getAdjacentNodes(), function (x) {
        return x.id == w.id;
      });
    }
    function processDirectedEdge(a, b) {
      ...
      _.each(b.getNeighbours(), function (c) {
        if (a.id != c.id && !areConnected(a, c)) {
          _.each(
            gn.getVertex(c.id).getParents(), //parents in gn is a superset of the parents in g
            function (d) {
    ...
    _.each(g.getEdges(), function (e) {
      if (
        e.directed != Graph.Edgetype.Directed &&
        !areConnected(gn.getVertex(e.v1.id), e.v2)
      )
        gn.addEdge(e.v1.id, e.v2.id, e.directed);
    });
    g.copyAllPropertiesTo(gn);
  ...
  contractComponents: function (g, components, includeSelfEdges) {
    ...
    if (typeof includeSelfEdges === "boolean")
      selfEdges = selfEdges.map(function () {
        return includeSelfEdges;
      });
    else if (_.isArray(includeSelfEdges))
      _.each(includeSelfEdges, function (t) {
        selfEdges[t] = true;
      });
...
var GraphGenerator = {
  ...
  setRandomNodes: function (g, p) {
    ...
    for (i = 0, j = 1, k = 2; i < prop.length; i++, j++, k++) {
      ...
      var minOthers =
        (p["min" + prop[j]] ? p["min" + prop[j]] : 0) +
        (p["min" + prop[k]] ? p["min" + prop[k]] : 0);
      if (
        !("max" + prop[i] in p) ||
    ...
    for (i = 0; i < vertices.length; i++) {
      ...
      var kind = null;
      if (q < pSource) {
        if (counts.Source < maxSource) kind = "Source";
      } else if (q < pSource + pTarget) {
        if (counts.Target < maxTarget) kind = "Target";
      } else if (q < pSource + pTarget + pLatentNode) {
        if (counts.LatentNode < maxLatentNode) kind = "LatentNode";
      }
      if (kind) {
        g["add" + kind](v);
        counts[kind]++;
    ...
    for (i = 0; i < prop.length; i++) {
      ...
      for (
        var existing = counts[prop[i]];
        existing < p["min" + prop[i]];
        existing++
      ) {
...
var ObservedGraph = Class.extend({
  init: function (graph) {
    ...
    _.each(
      Object.keys(this.event_mapping),
      function (k) {
        ...
        if (this.event_mapping[k]) {
          this[k] = (function (f, k) {
            return function () {
              ...
              _.each(this.event_listeners[this.event_mapping[k]], function (l) {
                l();
              });
              return r;
            };
          ...
          this[k] = (function (f) {
            return function () {
              return f.apply(graph, arguments);
            };
          })(f);
        }
      }
...
var GraphSerializer = {
  ...
  toDotVertexStatements: function (g) {
    var expandLabel = function (v, g) {
      ...
      if (properties.length > 0) {
        return (
          GraphSerializer.dotQuoteVid(v.id) + " [" + properties.join(",") + "]"
        );
      } else {
  ...
  toDotEdgeStatements: function (g) {
    var edgestat = [],
      es,
      eop,
      barewordre = _.each(g.edges, function (e) {
  ...
  toLavaan: function (g) {
    ...
    _.each(
      _.difference(_.pluck(g.getVertices(), "id"), _.keys(v_nonzero_degree)),
      function (vid) {
        r += vid + " ~~ " + vid + "\n";
      }
    );
  ...
  toTikz: function (g, precision) {
    ...
    for (i = 0; i < vv.length; i++) {
      r +=
        ...
        i +
        ") at (" +
        vv[i].layout_pos_x.toPrecision(precision) +
        "," +
        (-vv[i].layout_pos_y).toPrecision(precision) +
        ") {" +
        vv[i].id +
        "};\n";
      v_index[vv[i].id] = i;
    ...
    for (i = 0; i < ee.length; i++) {
      ...
      if (ee[i].layout_pos_x) {
        p1x = (
          (1 / 3) * ee[i].v1.layout_pos_x +
          (2 / 3) * ee[i].layout_pos_x
        ).toPrecision(precision);
        p1y = -(
          (1 / 3) * ee[i].v1.layout_pos_y +
          (2 / 3) * ee[i].layout_pos_y
        ).toPrecision(precision);
        p2x = (
          (1 / 3) * ee[i].v2.layout_pos_x +
          (2 / 3) * ee[i].layout_pos_x
        ).toPrecision(precision);
        p2y = -(
          (1 / 3) * ee[i].v2.layout_pos_y +
          (2 / 3) * ee[i].layout_pos_y
        ).toPrecision(precision);
        r +=
          "\\draw " +
  ...
  polynomialVariety: function (g, use_ids_as_labels, standardized) {
    ...
    for (i = 0; i < vv.length; i++) {
      //if( !standardized ){
      if (i == 0) {
        var parameters = GraphAnalyzer.trekRule(
          g,
          vv[i],
          vv[i],
          use_ids_as_labels,
      ...
      for (j = i + standardized; j < vv.length; j++) {
        ...
        var monomials = GraphAnalyzer.trekRule(
          g,
          vv[i],
          vv[j],
          use_ids_as_labels,
  ...
  toCausalEffectIgraphRCode: function (g) {
    ...
    _.each(gbidi.getEdges(), function (e) {
      ...
      edgesresult.push(
        GraphSerializer.quoteVid(e.v1.id) +
          "," +
          GraphSerializer.quoteVid(e.v2.id)
      );
      if (e.directed == Graph.Edgetype.Bidirected) {
    ...
    var result =
      edgesresult.length > 0
        ? "set.edge.attribute(graph = graph_from_edgelist(matrix(c( " +
          edgesresult.join(", ") +
          ' ),nc = 2,byrow = TRUE)), name = "description", index = c(' +
          bidirectional.join(", ") +
          '), value = "U")'
        : "make_empty_graph()";

    var withoutEdges = _.filter(g.getVertices(), function (v) {
      return !withEdges.get(v.id);
    });
    if (withoutEdges.length > 0)
      result =
        ...
        withoutEdges
          .map(function (v) {
            return GraphSerializer.quoteVid(v.id);
          })
          .join(", ") +
        ")";
  ...
  toCausalEffectRCode: function (g) {
    ...
    return (
      "causal.effect(y = " +
      nodeList(g.getTargets()) +
      ", x = " +
      nodeList(g.getSources()) +
      ", G = " +
      this.toCausalEffectIgraphRCode(g) +
      ")"
    );
  },
}; // eslint-disable-line
...
MPoly.prototype.toString = function (formatting) {
  ...
  return this.map(function (term, i) {
    ...
    if (factor != 1 || term.length == 1) temp.push(factor.toString());
...
var MPolyHelper = {
  ...
  parseSubTerm: function (st, negated, s) {
    //s: original input, only used for error messages
    st = st.trim();
    if (st.length == 0) throw "Empty sum term in " + s;
    //exponential factor notation requires space or * afterwards
    var takeFactor = /^([0-9.-]+([eE][0-9]+($|\s*[*]|\s+)|\s*[*]?))(.*)$/.exec(
      st.trim()
    );
    var factor = 1;
    if (takeFactor) {
    ...
    for (i = 0; i < monos.length; i++) {
      if (/[+*^-]/.test(monos[i][0]))
        throw "Monomials cannot contain math symbols. Separate monomials by space or *";
  ...
  parse: function (s) {
    if (s.indexOf("(") >= 0 || s.indexOf(")") >= 0)
      throw "Parentheses are not supported";
  ...
  assertIsMPoly: function (v) {
    if (!(v instanceof MPoly))
      throw "Expected multivariate polynomial, got: " + v;
  },
    ...
    if (!(res[0].length == 0 || res[0][0] == 0))
      for (i = 1; i < res.length; i++)
        if (
          res[i].length == 0 ||
          res[i][0] == 0 ||
          MPolyHelper.compareVariableOrder(res[i - 1], res[i]) == 0
        )
          break;
    if (i == res.length) return res; //all subterms are normalized
  ...
  variableToString: function (vidx) {
    if (vidx in MPolyHelper.variableToStringMap)
      return MPolyHelper.variableToStringMap[vidx];
    throw "Unknown variable ID: " + vidx;
  },
  variableFromString: function (v, mustExists) {
    if (v in MPolyHelper.stringToVariableMap)
      return MPolyHelper.stringToVariableMap[v];
...
(function (root) {
  ...
  function peg$parse(input, options) {
    ...
    var peg$FAILED = {},
      ...
      peg$c63 = function (m, id) {
        return (m === null ? "" : "-") + id.join("");
      },
      peg$c64 = '"',
      ...
      peg$c73 = function (v, v2) {
        return v2;
      },
      peg$c74 = function (v, rest) {
...
var DAGittyGraphView = Class.extend({
  init: function (el, graph, controller, obj) {
    ...
    this.impl.setEventListener(
      "vertex_marked",
      _.bind(function (v) {
        this.callEventListener("vertex_marked", [v]);
      }, this)
    );
  ...
  registerEventListeners: function (autofocus) {
    ...
    var myself = this,
      mycontainer = myself.getContainer();
    ...
    if (autofocus) {
      var f = function () {
        if (!mycontainer.contains(document.activeElement)) {
          mycontainer.focus();
        }
      };
  ...
  clickHandler: function (e) {
    ...
    this.last_click_g_coords = this.toGraphCoordinate(
      this.last_click_x,
      this.last_click_y
    );
  ...
  newVertex: function (n) {
    ...
    this.getController().newVertex(
      n,
      this.last_click_g_coords[0],
      this.last_click_g_coords[1]
    );
    this.closeDialog();
  },
  ...
  newVertexDialog: function () {
    var i = 1;
    if (!this.dialogOpen()) {
      while (this.getGraph().getVertex("v" + i)) {
        i++;
      }
  ...
  renameVertexDialog: function (vid) {
    ...
    if (!this.dialogOpen() && v) {
      this.openPromptDialog("rename variable:", v.id, function (n) {
        myself.renameVertex(n, v);
      });
    }
  },
  ...
  initializeCoordinateSystem: function (g) {
    ...
    if (bb) {
      ...
      if (this.isCoordinateSystemValid()) {
        return;
      }
      var min_x = Infinity,
        max_x = -Infinity,
  ...
  toScreenCoordinate: function (x, y) {
    return [
      ((x - this.bounds[0]) / (this.bounds[1] - this.bounds[0])) * this.width,
      ((y - this.bounds[2]) / (this.bounds[3] - this.bounds[2])) * this.height,
    ];
  },
  toGraphCoordinate: function (x, y) {
    return [
      (x / this.width) * (this.bounds[1] - this.bounds[0]) + this.bounds[0],
  ...
  drawGraph: function () {
    ...
    var g_an = GraphTransformer.ancestorGraph(
      GraphTransformer.backDoorGraph(this.getGraph())
    );

    switch (this.getViewMode()) {
      case "moral":
        if (
          this.graph.getSources().length > 0 &&
          this.graph.getTargets().length > 0
        ) {
      ...
      default:
        ...
        if (g.getSources().length > 0 && g.getTargets().length > 0) {
          ...
          switch (this.getBiasMode()) {
            case "causalodds":
              if (
                g.getSources().length == 1 &&
                g.getTargets().length == 1 &&
                g.getSelectedNodes().length == 1
              ) {
                g_bias = GraphTransformer.activeSelectionBiasGraph(
                  g,
                  g.getSources()[0],
                  g.getTargets()[0],
                  g.getSelectedNodes()
    ...
    var ean_ids = {};
    _.each(g_an.ancestorsOf(g_an.getSources()), function (v) {
      ean_ids[v.id] = 1;
    });
    var oan_ids = {};
    _.each(g_an.ancestorsOf(g_an.getTargets()), function (v) {
      oan_ids[v.id] = 1;
    });

    for (i = 0; i < vv.length; i++) {
      c = this.toScreenCoordinate(vv[i].layout_pos_x, vv[i].layout_pos_y);
      var vs = { id: vv[i].id, x: c[0], y: c[1], adjacent_edges: [], v: vv[i] };
      ...
      if (g.isSource(vv[i])) {
        ...
        vertex_type = "latent";
      } else if (ean_ids[vv[i].id] && oan_ids[vv[i].id]) {
    ...
    for (i = 0; i < ee.length; i++) {
      ...
      if (ee[i].layout_pos_x) {
        var s_coord = this.toScreenCoordinate(
          ee[i].layout_pos_x,
          ee[i].layout_pos_y
        );
        es.cx = s_coord[0];
      ...
      switch (ee[i].directed) {
        case 0:
          edgetypes.push("undirected");
          break;
        case 1:
          edgetypes.push("directed");
          break;
        case 2:
          edgetypes.push("bidirected");
          break;
      }
      ...
      this.edge_shapes.set(
        ee[i].v1.id + "\0" + ee[i].v2.id + "\0" + ee[i].directed,
        es
      );
...
var DAGittyController = Class.extend({
  init: function (obj) {
    ...
    if (obj.graph) {
      ...
      this.setGraph(
        GraphParser.parseGuess(obj.canvas.textContent || obj.canvas.innerText)
      );
    }
    ...
    this.view.setEventListener(
      "vertex_marked",
      _.bind(function (v) {
        _.each(this.event_listeners["vertex_marked"], function (l) {
          l(v);
        });
      }, this)
    );
  ...
  setGraph: function (graph) {
    ...
    var myself = this;
    this.observed_graph.observe("change", function () {
      myself.graphChanged();
    });
    if (this.view) {
  ...
  graphChanged: function () {
    this.redraw();
    var g = this.getGraph();
    _.each(this.event_listeners["graphchange"], function (l) {
      l(g);
    });
  },
  graphLayoutChanged: function () {
    var g = this.getGraph();
    _.each(this.event_listeners["graphlayoutchange"], function (l) {
      l(g);
    });
  },
  ...
  toggleEdgeBidi: function (v1, v2) {
    ...
    if (
      v1.incomingEdges.length + v1.outgoingEdges.length > 1 &&
      _.isNumber(v1.layout_pos_x) &&
      _.isNumber(v2.layout_pos_x)
    ) {
      //curve the edge
  ...
  toggleEdgeBetween: function (v1, v2) {
    ...
    if (i >= edgeType.length) {
      ...
      newe = this.getObservedGraph().addEdge(
        v1,
        v2,
        edgeType[(i + 1) % edgeType.length]
      );
    }
  ...
  renameVertex: function (vold, vnew) {
    if (this.getGraph().getVertex(vold) && !this.getGraph().getVertex(vnew)) {
      this.getObservedGraph().renameVertex(vold, vnew);
    }
  },
  newVertex: function (id, x, y) {
    this.getObservedGraph().addVertex(
      new Graph.Vertex({ id: id, layout_pos_x: x, layout_pos_y: y })
    );
  },
  ...
  setVertexProperty: function (_v, p) {
    ...
    if (v) {
      ...
      _.each(
        ["Source", "Target", "AdjustedNode", "LatentNode", "SelectedNode"],
        function (pc) {
  ...
  Math: {
    ...
    svgEdgeAnchor: function (el, reverse, edgetype) {
      ...
      try {
        ...
        if (
          edgetype == Graph.Edgetype.Undirected ||
          (edgetype == Graph.Edgetype.Directed && reverse == 0)
        ) {
          elongate = 1;
        }
        return [
          svgpoint.x * elongate + el[v[0]].x,
...
var GraphGUI_SVG = Class.extend({
  init: function (canvas_element, width, height, op) {
    ...
    _.map(
      ["touchmove", "mouseup", "mousemove", "mouseleave", "click"],
      function (x) {
  ...
  addEdgeEventListeners: function (el) {
    ...
    el.dom.addEventListener(
      "mouseover",
      _.bind(function () {
        this.last_hovered_element = { edge: el.e };
      }, this)
    );

    el.dom.addEventListener(
      "mouseout",
      _.bind(this.unsetLastHoveredElement, this)
    );
  },
    ...
    for (var i = 0; i < el.dom.childNodes.length; i++) {
      if (el.dom.childNodes.item(i).getAttribute("class") == "arrowfront") {
        ...
        el.dom.childNodes
          ...
          .setAttribute(
            "transform",
            "translate(" + el.x2 + "," + el.y2 + ") rotate(" + afront + ")"
          );
      }
      if (el.dom.childNodes.item(i).getAttribute("class") == "arrowback") {
        ...
        el.dom.childNodes
          ...
          .setAttribute(
            "transform",
            "translate(" + el.x1 + "," + el.y1 + ") rotate(" + aback + ")"
          );
      }
    }
  },
  ...
  addVertexEventListeners: function (el) {
    ...
    el.dom.addEventListener(
      "mouseover",
      _.bind(function () {
        this.last_hovered_element = { vertex: el.v };
      }, this)
    );
  ...
  markVertexShape: function (el) {
    el.previous_stroke = el.dom.firstChild.getAttribute(
      "stroke",
      this.getStyle().style.node.stroke
    );
    el.previous_stroke_width = el.dom.firstChild.getAttribute(
      "stroke-width",
      this.getStyle().style.node["stroke-width"]
    );
    el.dom.firstChild.setAttribute(
      "stroke-width",
      3 * el.previous_stroke_width
    );
    ...
    for (var i = 0; i < shape_array.length; i++) {
      ...
      if (shape_array[i].pathtemplate) {
        ...
        shape_array[i].pathtemplate = shape_array[i].pathtemplate
          ...
          .replace(/\[dy\]/g, h)
          .replace(/\[ry\]/g, h / 2);
        shape_array[i].dom.firstChild.setAttribute(
          "d",
          shape_array[i].pathtemplate
        );
