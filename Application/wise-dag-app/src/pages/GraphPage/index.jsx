...
function findNodeInHierarchy(name, nodes) {
for (const node of nodes) {
if (node.name === name) {
return node;
}
if (node.children) {
const found = findNodeInHierarchy(name, node.children);
...
// Find the node object in the hierarchy
const nodeObj = findNodeInHierarchy(nodeName, hierarchy);
if (nodeObj) {
// Get all children (includes the node itself)
const childNames = getAllChildNames(nodeObj);
// Add them to selectedNodes (union them in)
setSelectedNodes((prevSelected) => [
...new Set([...prevSelected, ...childNames]),
]);
}
}
return newSet;
...
? prev.filter((n) => n !== nodeName) // remove ONLY this node
: [...prev, nodeName] // add ONLY this node
);
};
...
selectedNodes.includes(node.name) ? 'bg-red-200' : ''
}`}
>
{node.name}
</span>
